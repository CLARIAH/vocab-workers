...
from vocab.cmdi import with_version_and_dump, get_file_for_id
from vocab.util.rdf import load_cached_into_graph
from vocab.util.xml import ns, ns_prefix, voc_root, grab_first, read_root, write_root
...
def summarizer(id: str) -> None:
    for record, version, cached_version_path in with_version_and_dump(id):
        try:
            ...
            write_summary_statements(id, version.version, summary)
        except Exception as e:
            log.error(f'Failed to summarize for {id} and version {version.version}: {e}')
...
def write_summary_statements(id: str, version: str, summary: Summary) -> None:
